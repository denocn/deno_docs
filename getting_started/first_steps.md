...
本文档假设您对 JavaScript 有一定的了解，特别是异步编程 `async`/`await`。如果您是首次接触 JavaScript，在您尝试使用 Deno 之前，您需要先学习掌握
...
3. 我们将响应体解析为
   ...
   [`Uint8Array`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) 以存储在
   `body` 常量中
...
Deno 还提供了不是来自 web 的 api。这些都包含在 `Deno` global 中。您可以在 [doc.deno.land](https://doc.deno.land/deno/stable/~/Deno) 上找到这些 API
的文档。
...
> Note for Windows users: netcat is not available on Windows. Instead you can use the built-in telnet client. The telnet
> client is disabled in Windows by default. It is easy to enable however: just follow the instructions
